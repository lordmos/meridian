...
Four preset visual languages Meridian can render a project in. Each style defines the full visual system — palette, logo variant, typography stack, VitePress theme vars, icon style — not just colors.

All four share the same **M + compass** motif (outer marker + four cardinal ticks + center letter). Each style interprets that motif in its own aesthetic register.
...
| `preview.svg` | Same as `hero.svg` but with `M` substituted — used for docs and inline embedding |
| `palette.svg` | 240×48 three-swatch preview for README gallery cards |
| `screenshot.png` | Real VitePress homepage rendered in this style, for README gallery |
| `style.md` | YAML frontmatter (palette / typography / VitePress theme vars) + prose fit guide |

The `screenshot.png` files are regenerated by `scripts/screenshot-styles.sh` (see that script for the demo VitePress project in `templates/styles/_demo/`).
